...
class OrderByFunctions():
    ...
    def displayOrderBy( self, actionPath ):
        try:
            ...
            action = "plugin://plugin.program.video.node.editor?type=editOrderBy&actionPath=" + actionPath + "&content=" + content + "&default=" + translated[ 0 ][ 1 ]
            ...
            action = "plugin://plugin.program.video.node.editor?type=editOrderByDirection&actionPath=" + actionPath + "&default=" + translated[ 1 ][ 1 ]
            xbmcplugin.addDirectoryItem( int(sys.argv[ 1 ]), action, listitem, isFolder=False )
